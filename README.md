...
| Fama-French five-factor and momentum daily returns | `data-raw/empirics/ff/F-F_Research_Data_5_Factors_2x3_daily_CSV.zip`; `data-raw/empirics/ff/F-F_Momentum_Factor_daily_CSV.zip`; `data/empirics/ff/ff_six_factors_daily.rds` | Raw zips and transformed RDS are tracked | Download URLs are encoded in `inst/empirics/scripts/data_fama_french_daily_factors.R` and point to the Kenneth French Data Library. |
...
| `data-raw/empirics/ff/F-F_Research_Data_5_Factors_2x3_daily_CSV.zip` | Kenneth French Data Library | ZIP/CSV | Daily market, SMB, HML, RMW, CMA, and risk-free returns. |
| `data-raw/empirics/ff/F-F_Momentum_Factor_daily_CSV.zip` | Kenneth French Data Library | ZIP/CSV | Daily momentum-factor returns. |
...
| `data/empirics/ff/ff_six_factors_daily.rds` | Derived from Fama-French files | RDS | Merged daily market, characteristic, momentum, and risk-free factor series. |
...
| --- | --- | --- |
| `data/empirics/ff/*.csv` | `inst/empirics/scripts/data_sorts_2x3_daily.R`; `inst/empirics/scripts/data_fama_french_daily_factors.R` | Optional CSV exports of transformed Fama-French returns, losses, metadata, and daily factors. |
...
- `Rscript scripts/run_empirics.R` completed in about six minutes, including
  1,000 full-pipeline block-bootstrap replications for each portfolio panel.
...
| `inst/empirics/scripts/portfolio_coexceedances_agca.R` | `20260707` | Bootstrap AVE summaries; paired coexceedance bootstrap uses `20260708` for Fama-French and `20260709` for OSAP. |
...
| `inst/empirics/scripts/portfolio_factor_attribution_agca.R` | `20260826` | Joint factor-exposure and AGCA moving-block bootstrap. |

The simulation scripts accept command-line overrides such as `--seed=`,
`--bootstrap-reps=`, `--skip-bootstrap`, `--reps=`, `--n=`, and `--k=`.
The factor-attribution script accepts `--factor-seed=`,
`--factor-bootstrap-reps=`, `--factor-block-length=`, and
`--skip-factor-bootstrap`.
The coexceedance script accepts `--comparison-only` to regenerate the
Fama-French/OSAP comparison and conditional diagnostic bands without
rerunning or overwriting the Fama-French sensitivity analyses. It saves
`panel_comparison_*.csv`, `fig_emp_portfolio_coexceedance_survival.pdf`, and
`fig_emp_portfolio_coexceedance_difference.pdf`, and stages only figures
referenced by the manuscript. When existing Fama-French or factor results
are available, it checks agreement with those fitted curves; it also
checks that the Fama-French conditional bootstrap is unchanged.
...
| `scripts/run_empirics.R` | Run portfolio tail-function diagnostics, coexceedance analysis, factor attribution, reporting figures, and time-dependence robustness checks. | The four empirical result directories under `inst/empirics/results/`. |
...
| `inst/empirics/scripts/data_fama_french_daily_factors.R` | Downloads/parses Fama-French five-factor and momentum daily returns. |
...
| `inst/empirics/scripts/portfolio_coexceedances_agca.R` | Compares observed, full angular, and AGCA coexceedance curves for Fama-French and OSAP, with paired conditional bootstrap bands, and computes additional Fama-French diagnostics. |
...
| `inst/empirics/scripts/portfolio_factor_attribution_agca.R` | Maps conventional factor-exposure profiles into the AGCA tangent space, evaluates joint and factor-specific attribution for both portfolio panels, and bootstraps the complete two-step estimator. |
...
`../jrssb_paper/figures/` when that manuscript directory exists. The
coexceedance script stages its seven manuscript figures there as well, and the
factor-attribution script stages its two figures there. These staging steps are
optional for reproducing the package outputs.

## Output Mapping

The saved result directories currently contain 176 CSV/PDF files: 100 CSV files
and 76 PDF files.
...
| `inst/empirics/results/portfolio_coexceedances_agca/` | `inst/empirics/scripts/portfolio_coexceedances_agca.R` | 31 | Two-panel coexceedance comparison, paired conditional bootstrap bands, reconstruction errors, and Fama-French robustness diagnostics. |
| `inst/empirics/results/portfolio_factor_attribution_agca/` | `inst/empirics/scripts/portfolio_factor_attribution_agca.R` | 12 | Factor exposures, geometric attribution, hybrid residual paths, robustness checks, full-pipeline bootstrap summaries, and figures. |
...
Kenneth French Data Library. Daily 2 x 3 portfolio and factor archives.
Download URLs are listed in `inst/empirics/scripts/data_sorts_2x3_daily.R` and
`inst/empirics/scripts/data_fama_french_daily_factors.R`.
